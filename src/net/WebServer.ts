 ...
 */
 ...
 export class WebServer
 {
     ...
     public get viewInterfaceNames(): string[] { return (Globals.fs.readdirSync(this.viewInterfacesRoot) as string[]).filter(i => i.toLowerCase().endsWith(this.enviroment)).map(i => i.substring(0, i.length-3));}
     
     /** Endpoint management object  */
     private endpoint: EndpointManager | undefined;
 
     private get enviroment(): string { return __filename.split(".").pop() as string}
     ...
     public async getViewPrototypes() : Promise<Map<string, {[key: string]: IWebRequest} | undefined>> 
     {
         ...
         const extension: string = __filename.split(".").pop() as string;
         Logger.log(`Loading ${extension} enviroment`);
         ...
         for(const ejsN of this.ejsViewNames)
         {
             ...
             if(interfaceName)
             {
                 try
                 {
                     const modulePrototype = Object.assign({}, await import(path.join(this.viewInterfacesRoot, `${interfaceName}.${extension}`)));
